...
public class PhoneWindowManager implements WindowManagerPolicy {
        ...
        if (keyCode == KeyEvent.KEYCODE_HOME) {
            ...
            if (repeatCount == 0) {
                ...
                if (mOmniSwitchRecents) {
                    AicpUtils.hideOmniSwitchRecents(mContext, UserHandle.CURRENT);
                }
                if (mHomeDoubleTapPending) {
                    mHomeDoubleTapPending = false;
    ...
    private void preloadRecentApps() {
        ...
        if (mOmniSwitchRecents) {
            AicpUtils.preloadOmniSwitchRecents(mContext, UserHandle.CURRENT);
            return;
        }
    ...
    private void showRecentApps(boolean triggeredFromAltTab, boolean fromHome) {
        if (mOmniSwitchRecents) {
            if (fromHome) {
              AicpUtils.restoreHomeStack(mContext, UserHandle.CURRENT);
            } else {
              AicpUtils.toggleOmniSwitchRecents(mContext, UserHandle.CURRENT);
            }
        } else {
            mPreloadedRecentApps = false; // preloading no longer needs to be canceled
            StatusBarManagerInternal statusbar = getStatusBarManagerInternal();
