 ...
 */
@SysUISingleton
public class AicpSettingsServiceImpl extends AicpSettingsService {
    private static final String TAG = "AicpSettingsService";
    private final Observer mObserver = new Observer();
    // Map of Uris we listen on to their settings keys.
    private final ArrayMap<Uri, String> mListeningUris = new ArrayMap<>();
    // Map of settings keys to the listener.
    private final HashMap<String, Set<AicpSettingsObserver>> mObserverLookup = new HashMap<>();
     ...
     */
    @Inject
    public AicpSettingsServiceImpl(Context context, @Main Handler mainHandler,
    ...
    @Override
    public void addStringObserver(AicpSettingsObserver observer, String... keys) {
        for (String key : keys) {
    ...
    @Override
    public void addIntObserver(AicpSettingsObserver observer, String... keys) {
        for (String key : keys) {
            addIntObserver(observer, key);
        }
    }

    private void addObserver(AicpSettingsObserver observer, String key) {
        if (!mObserverLookup.containsKey(key)) {
            mObserverLookup.put(key, new ArraySet<AicpSettingsObserver>());
        }
    ...
    private void addStringObserver(AicpSettingsObserver observer, String key) {
    ...
    private void addIntObserver(AicpSettingsObserver observer, String key) {
    ...
    @Override
    public void removeObserver(AicpSettingsObserver observer) {
        for (Set<AicpSettingsObserver> list : mObserverLookup.values()) {
    ...
    private void reloadSetting(Uri uri) {
        ...
        Set<AicpSettingsObserver> observers = mObserverLookup.get(key);
        if (observers == null) {
            return;
        }
        if (mStringSettings.contains(key)) {
            String value = Settings.System.getStringForUser(mContentResolver, key, mCurrentUser);
            for (AicpSettingsObserver observer : observers) {
                observer.onStringSettingChanged(key, value);
            }
        }
    ...
    private void reloadAll() {
        for (String key : mObserverLookup.keySet()) {
            if (mStringSettings.contains(key)) {
                String value = Settings.System.getStringForUser(mContentResolver, key, mCurrentUser);
                for (AicpSettingsObserver observer : mObserverLookup.get(key)) {
                    observer.onStringSettingChanged(key, value);
                }
            }
            if (mIntSettings.contains(key)) {
